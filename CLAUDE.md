...
**Before raising a PR:**
- All CI checks must pass: build-and-test, clang-build-and-test, windows-build-and-test, sanitize, coverage, tidy, cppcheck, format
...
- PRs require all status checks to pass before merging: build-and-test, clang-build-and-test, windows-build-and-test, sanitize, coverage, tidy, cppcheck, format
...
| `msvc-debug` | MSVC build — Windows portability check (CI only, Windows-only preset) |
